...
Furthermore, we consider an algorithm to achieve \textit{real-time plane detection} ($RT_{calc}$) if its
plane detection step runs faster than $1s$.
...
\paragraph{2D-3D-S}
% FIXME das hier an der tabelle anpassen!!!
\textbf{\textcolor{red}{an der tabelle anpassen}}
...
the minimum number of samples per leaf node $\varepsilon$, the maximum percentage of outliers per plane $MOR$, and
...
\paragraph{FIN}
We performed tests with different parameterizations, ranging as follows. 
