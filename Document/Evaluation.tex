...
We calculate the F1-Score, as it is the harmonic mean of the \textit{Precision} and the \textit{Recall}. In practice, this means that an algorithm
...
and longer times for the most recent point cloud. It is, therefore, necessary to analyze the
...
This separation allows for an in-depth analysis of the results. 
...
\textbf{\textcolor{red}{die reihenfolge dieser subsection fühlt sich off an weeeh}}
...
the \textit{F1-Score} of each algorithm were calculated. The computation times were measured and divided into pre-processing, plane detection,
...
\paragraph{Accuracy}
RSPD has the overall highest accuracy with a \textit{Precision} of ${\sim}85\%$, a \textit{Recall} of ${\sim}90\%$, and an F1-Score of ${\sim}87\%$.
OPS supersedes RSPD with a \textit{Precision} value of ${\sim}89\%$ but scores significantly lower \textit{Recall} and F1-Score values.
3D-KHT yields \textit{Precision}, \textit{Recall} and F1-Score results in the range of ${\sim}71\%$ and ${\sim}79\%$.
OBRG has a high \textit{Precision} value of ${\sim}81\%$, however its \textit{Recall} and F1-Score values are the lowest out of all algorithms
with ${\sim}67\%$ and ${\sim}71\%$, respectively.

\paragraph{Average Time }
\label{par:2D-3D-S-time}
With an average of 0.71 seconds spent in pre-processing, and an average of 1.03 seconds spent in plane detection,
3D-KHT only needs an average total of 1.74 seconds to process an entire point cloud and thereby achieves the lowest calculation times.
...
to falling under the threshold of $1s$ for \textit{real-time plane detection}.
...
\begin{figure}[H]
    ...
    \caption[Time per Cloud size 2D-3D-S]{The time spent in pre-processing $t_{pre}$, plane detection $t_{calc}$,
        post-processing $t_{post}$, and the total calculation time $t_{tot}$ per point cloud file size of the 2D-3D-S dataset.
...
Since the post-processing phase operates on the unrefined regions returned by the plane detection phase, the curve behaves similarly,
supporting the sub-optimal parameterization argument. \textbf{\textcolor{red}{schwierig das zu behaupten. SISO prinzip}}
...
$0.03-0.04$ over the $1s$ threshold.
...
The highest values are written in bold for \textit{Precision, Recall,} and the \textit{F1-Score}, as are the lowest times of each calculation step.
...
3D-KHT and OBRG achieve a similar \textit{Precision}, but for \textit{Recall} and F1-Score, however, 3D-KHT has
...
The average calculation times of OBRG show non-\textit{real-time} applicability.
...
\begin{figure}[H]
    ...
    \caption[Time Results Auditorium]{Time spent in pre-processing $t_{pre}$, plane detection $t_{calc}$, post-processing
        ...
        Note, that both y-axes are scaled logarithmically to the base of ten.}
    \label{fig:dynaudi}
\end{figure}
...
OPS has the highest \textit{Precision} value, RSPD yields the highest \textit{Recall} and F1-Score, and 3D-KHT has the lowest
...
Due to \textit{Recall} and the F1-Score, RSPD has the highest overall accuracy in both experiments.
