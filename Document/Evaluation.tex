...
\section{Protocol}
\textbf{\textcolor{red}{hier besser formulieren}}
This work aims to determine the applicability of plane detection algorithms for AR/VR systems in a realistic environment. For this decision, we uniformly compare the algorithms selected in
...
\paragraph{2D-3D-S}
We use the parameters of the provided implementation\footnote{\href{https://github.com/abnerrjo/PlaneDetection}{https://github.com/abnerrjo/PlaneDetection}} for the \textit{2D-3D-S} experiment. These parameters include the maximum octree level $l_O$,
...
We considered the reported parameterizations in~\cite[Tables~1,4,7]{Vo_Truong-Hong_Laefer_Bertolotto_2015}. However, since the authors used three large outdoor scenes for their evaluation, they would likely not be optimal for smaller, indoor scenes. We confirmed this assumption based on a small set of empiric experiments with scenes from the \textit{2D-3D-S} dataset, wherein we compared the results for the reported parameterization and the one we determined.
...
\begin{table}[H]
    \centering
    \caption[Average 2D-3D-S Results]{Average results of each algorithm over the \textit{2D-3D-S} dataset. The right half of the inner columns shows the average time spent in
...
%         and post-processing $t_{post}$ per point cloud file size of the \textit{2D-3D-S} dataset. Note that both y-axes are
...
\begin{figure}[]
    ...
    \caption[Time Results 2D-3D-S]{The time spent in pre-processing $t_{pre}$, plane detection $t_{calc}$,
        post-processing $t_{post}$, and the total calculation time $t_{tot}$ per point cloud size of the \textit{2D-3D-S} dataset.
...
maximum size of the FIN experiment (${\sim}0.75\cdot10^6$) is very small, compared to largest scene of the \textit{2D-3D-S} dataset (${\sim}9\cdot10^6$).
...
3D-KHT is \textit{Real-Time Plane Detection} applicable in the \textit{2D-3D-S} experiment and \textit{totally real-time} applicable
