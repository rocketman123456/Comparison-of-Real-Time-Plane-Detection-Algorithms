...
In the following chapters, we often refer to different data representations. This section provides detailed information about these data representations while differentiating between the input and output of a plane detection algorithm. In this work, not that an algorithm's input is equivalent to the output of the sensors or the SLAM algorithm.

\subsection{Common Input Types}
\label{subsec:input}
As described in Chapter~\ref{chap:Introduction}, specialized sensors are needed to record the environment. Like the exported data, the input for the plane detection algorithm differs.
...
Moreover, the D455 has an additional RGB and infrared (IR) sensor. The combination of IR and two imaging sensors is used to produce depth images.
The T265 has a much wider field of view (FOV) of 163° than the D455, which only has an FOV of 111°. The maximum frames per second (FPS) of the D455 is determined by the individual FPS of both its imaging sensors, and its IR sensor, which equals $90$. It is worth noting that the FPS decreases with increasing sensor resolution and that 90 FPS is only reached with a resolution of 640x480. In contrast, the T265 has a maximum FPS of $30$.
Furthermore, both cameras have an integrated Inertial Measurement Unit (IMU) which is used to compute their position in combination with visual input.
...
This tuple is also called a \textit{Hough Space} in this context. Votes are cast on the corresponding value of $\theta$, depending on the number of inliers within a specific \textit{Hough Space} $(r_i,\theta_i)$. The map that connects the
...
The Kinect~\cite{Oehler_Stueckler_Welle_Schulz_Behnke_2011} dataset is a set of 30 organized point clouds. The recording was performed, as the
