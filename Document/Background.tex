...
In the following chapters, we often refer to different data representations. This section provides detailed information about these data representations while differentiating between the \textit{input} and \textit{output} of a plane detection algorithm. In this work, not that an algorithm's \textit{input} is equivalent to the \textit{output} of the sensors or the SLAM algorithm.
...
\begin{table}[H]
    \centering
    \caption[Intel RealSense T265 \& D455 Specification]{Intel RealSense T265 and D455 camera specifications. The shown \textit{resolution} and \textit{FPS} are the respective maximum values. More information and the complete Datasheets can be found on https://www.intelrealsense.com/.}
    \resizebox{\textwidth}{!}{%
...
\paragraph{Real-Time Appearance-Based Mapping}
...
Region Growing (RG) methods are often used in the field of image or point cloud segmentation \cite{Proença_Gao_2018, Vo_Truong-Hong_Laefer_Bertolotto_2015}.
...
\begin{figure}[H]
    ...
    \caption[SYNPEB Dataset]{The \textit{SYNPEB} room, taken from \href{http://synpeb.cs.uni-freiburg.de/}{http://synpeb.cs.uni-freiburg.de/}.}
    \label{fig:SYNPEB}
\end{figure}
