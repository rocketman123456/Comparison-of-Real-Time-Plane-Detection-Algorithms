...
\begin{figure}[!h]
    ...
    \caption[RTAB-MAP Block Diagram]{Block diagram of RTAB-MAP's main node. Note, that the \textit{Point Cloud} returned by the \textit{Global Map Assembling} 
...
\textit{2D-3D-S}~\cite{armeni_cvpr16} was recorded in three different buildings and divided into six distinct areas, including 272 different scenes. A detailed statistic of the included scene types can be found in Table~\ref{tab:stanfordStats}.
...
The dataset is saved in \textit{ASTM E57}~\cite{huber2011astm}, which is a general-purpose file format usually used for the
