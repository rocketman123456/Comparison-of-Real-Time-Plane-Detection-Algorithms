...
The original motivation behind the Hough transform was the detection of lines in images. To detect the best fitting line over a set of 2d points, all points are sequentially processed via a voting procedure.
Given a point $p$, multiple lines with different orientations are fit through $p$. Because an orientation that is parallel to the y-axis would lead to an infinite slope, the Hesse-normalform is chosen as the primary line representation. 
Therefore, an individual line is represented as a pair $(r, \theta)$, whereas $r$ is the orthogonal distance from the origin to the plane, and $\theta$ is the angle between the x-axis and the line that connects the origin to the closest point on the line. 
This pair is also called a \textit{Hough Space} in this context. Votes are cast on the corresponding $\theta_i$ value, depending on the number of inliers within a specific \textit{Hough Space} $(r_i,\theta_i)$. The map that connects the
votes to each $\theta$ is called an \textit{accumulator}.
Finally, the best fitting line is determined by the number of votes that each $\theta$ received.
