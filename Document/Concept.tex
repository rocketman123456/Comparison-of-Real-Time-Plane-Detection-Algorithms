...
For example, judging an algorithm that focuses on indoor plane detection based on its ability to detect moving cars in an urban environment would be pointless.
...
As mentioned at the bigging of this chapter, we also need an appropriate dataset for the evaluation.
Through extensive research of current literature, we compiled a list of popular datasets (see Table~\ref{tab:datasets}).
% FIXME irgendwo muss hier noch hin, dass ich ebenen als ground truth gerne hätte
...
Consequently, we create a ground truth that aligns with the aim of this work. We outline the details thereof in Section~\ref{sec:gtseg}.  
