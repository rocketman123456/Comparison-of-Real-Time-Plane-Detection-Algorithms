...
\begin{table}[H]
    ...
    \begin{tabular}{c|c|c}
        ...
        \textbf{PlaneRecNet} \cite{Xie_Shu_Rambach_Pagani_Stricker_2022} & RGBI                & reconstructed scene   \\  %& Y                       \\ %& Y                     \\ \hline
...
To enable a precise evaluation, we subdivide the plane detection algorithms into three phases:
...
we refer the reader to the Subsections~\ref{subsec:bg-rspd}-\ref{subsec:bg-obrg} for information on the plane detection
step.
...
Since we focus on the detection of planes in indoor environments, \textit{Leica} also ceases to be an option. 
Thus, we choose 2D-3D-S as the dataset for the evaluation.
...
Because some steps can be covered by previous steps in the AR/VR system, i.e., by the sensor or the SLAM algorithm, 
