...
% FIXME generic view on AR + VR system
AR and VR Systems often integrate plane detection into their software.  
AR und VR Applikationen nehmen oft gebrauch von plane detection. \textcolor{red}{To provide examples,} \cite{Carranza_Estrella_Zaidi_Carranza_2021} use AR technology and plane detection 
to aid the movement of visually impaired people. % FIXME beispiele
Figure~\ref{fig:concept} shows a generic block diagram of such a VR/AR system including plane detection.
First, the environment is recorded by a specialized sensor which is usually a camera([1]). A SLAM algorithm then integrates the new data into its already existing map([2]). The map, in form of a point cloud,
...
The application itself could, e.g., then visualize the detected planes or guide a visually impaired person by providing voice commands.

% FIXME Describe the building blocks in there
% SECTION Point out that the decision on the "best" PDA is not clear
As previously stated in Chapter~\ref{chap:Introduction}, most published algorithms are inherently incomparable. The incomparability could stem from the method of evaluation,
i.e., the metrics, the chosen dataset and even how the metrics are calculated. Additionally, the algorithms themself are a possible reason for incomparability, as they often differ
in aspects like type of input, the format of detected planes and the amount of necesary pre- and postprocessing.

An algorithm that has lower reported runtimes than another might in reality run slower because some variables like the amount of noise changed.
Therefore, the "best" plane detection algorithm selection is non-trivial, impossible even, as previously mentioned in Chapter~\ref{chap:Introduction}.
% FIXME vertiefen: (aus INTRO) "Problem an letzterem: nicht möglich einen einheitlich besten algorithmus auszuwählen"  
% FIXME Tell me why that is not clear!


%!SECTION 
% !!SECTION -
% SECTION - CONCEPT PDA: get me going on those PDA 
...
As mentioned at the beginning of this chapter, the algorithms differ in certain aspects. 

\subsection{Criteria}
We outline these aspects in the following paragraphs. Furthermore, we declare the most appropriate values of each criterion for the context of this work.
