...
% As previously stated in Chapter~\ref{chap:Introduction}, most published algorithms are inherently incomparable. The incomparability could stem from the method of evaluation, i.e., the metrics, 
% the chosen dataset, and even how the metrics are calculated. Additionally, the algorithms themself are a possible reason for incomparability, as they often differ
% in aspects like the expected input type, the format of detected planes, and the amount of necessary pre-and/or postprocessing.
% Therefore, selecting the "best" plane detection algorithm from the reported metrics alone is not possible. 
...
% Addressing \ref{enum:pda}, we first present a list of the current state-of-the-art plane detection algorithms, which can be seen in Table~\ref{tab:algos}. 
