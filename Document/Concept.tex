...
\begin{figure}[H]
    ...
    \caption[AR/VR System]{The procedure of the plane detection process. The specialized sensor records data ([1]), which is passed to
...
\begin{itemize}
    ...
    \item unorganized or structured point cloud (OPC),
...
\begin{table}[H]
    ...
    \caption[State-of-The-Art Plane Detection Algorithms]{A list of Plane Detection Algorithms compiled by reviewing the current literature. The algorithms are clustered by their type of input.
        The Section column provides the placement within this work. We refer to Subsection~\ref{subsec:criteria} for details regarding the specific namings. In the Hardware column, note that GPU implies the usage of the CPU.}
    \resizebox{\textwidth}{!}{%
...
The Format of a scene in a given dataset corresponds directly to the Input Type criterion for the selection of algorithms (see Paragraph~\ref{par:input}).
...
A ground truth can represent the detectable planes in a scene.
...
Therein, all, or most, objects of a scene are assigned specific object classes, e.g., "Wall", "Ceiling", "Table", or "Cup".
The ground truth provides a labeling of these objects. This labeling can take the form of annotated 3D bounding boxes or sets of
pixels in the input image.

Lastly, some datasets provide a GT that focuses on the trajectory of the recording sensor.

Naturally, this classification can only apply to datasets that include a grund truth.
...
Note that SYNPEB and SegComp are synthetic datasets and, therefore, are neither indoor nor outdoor.
...
Furthermore, since we give special focus to the real-world applicability of plane detection algorithms, we must evaluate them on realistic datasets, thereby excluding all datasets except \textit{2D-3D-S} and Leica.
Lastly, we are especially interested in realistic indoor environments, as motivated by Chapter~\ref{chap:Introduction}.
Therefore, Leica ceases to be an option and we subsequently choose 2D-3D-S as the dataset for the evaluation.


Nevertheless, we cannot use the ground truth included in \textit{2D-3D-S} because it represents the segmented scene at the level of objects~\cite[Section~4.1]{2017arXiv170201105A} instead of focusing on layers within the scene.
...
To perform a thorough comparison between the \textit{FIN} and \textit{2D-3D-S}, we record a scene for each of the following scene types:
\begin{itemize}
    \item auditorium
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.5\textwidth}
        ...
        \label{fig:finhw}
    \end{subfigure}
    \caption[FIN Dataset]{The recorded point clouds for each scene type: (a) $auditorium$, (b) c$onference room$, (c) $office$ and (d) $hallway$.
...
Finally, as mentioned in the beginning of Chapter~\ref{chap:Concept}, we must define the meaning of \textit{real-time} to determine the \textit{real-time} applicability of a plane detection algorithm.
...
Being a subset of \textit{totally Real-Time} applicability, \textit{Real-Time Plane Calculation} determines the \textit{real-time} applicability if the processing time of an algorithm
